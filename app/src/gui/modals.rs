use eframe::egui::{Color32, Context, Frame, RichText, ScrollArea};
...
pub fn about(ctx: &Context) -> Modal {
    ...
    modal.show(|ui| {
        ...
        modal.frame(ui, |ui| {
            modal.body(ui, "A collaborative effort between 4JX and ABHI-devhome.");

            ui.vertical(|ui| {
                Frame::NONE.inner_margin(ModalStyle::default().body_margin).show(ui, |ui| {
                    ui.label(RichText::new("Original Author:").strong());
                    if ui.link("https://github.com/4JX/L5P-Keyboard-RGB").clicked() {
                        let _ = open::that("https://github.com/4JX/L5P-Keyboard-RGB");
                    }

                    ui.add_space(5.0);

                    ui.label(RichText::new("Contributor (Custom Editor & Modern UI):").strong());
                    if ui.link("https://github.com/ABHI-devhome").clicked() {
                        let _ = open::that("https://github.com/ABHI-devhome");
                    }

                    ui.add_space(10.0);

                    ui.label("Something's not working?");
                    let issue_url = "https://github.com/4JX/L5P-Keyboard-RGB/issues";
                    if ui.link(issue_url).clicked() {
