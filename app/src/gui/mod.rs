...
pub struct App {
    ...
    theme: Theme,
    toasts: Toasts,
    effect_panel_tab: EffectPanelTab,
}
...
impl App {
    pub fn new(output: OutputType, has_tray: Arc<AtomicBool>, visible: Arc<AtomicBool>) -> Self {
        ...
        let mut app = Self {
            ...
            effect_panel_tab: EffectPanelTab::default(),
        };
...
impl App {
    ...
    fn show_ui_elements(&mut self, ctx: &Context, ui: &mut eframe::egui::Ui) {
        let spacing = self.theme.spacing.large;
        let total_size = ui.available_size();

        ui.horizontal(|ui| {
            ui.spacing_mut().item_spacing.x = spacing;

            // --- LEFT COLUMN: Profiles & Main Controls ---
            ui.allocate_ui_with_layout(Vec2::new(300.0, total_size.y), Layout::top_down(Align::Min).with_cross_justify(true), |ui| {
            ...
            // --- RIGHT COLUMN: Effects & Custom Editor ---
            ui.allocate_ui_with_layout(Vec2::new(ui.available_width(), total_size.y), Layout::top_down(Align::Min).with_cross_justify(true), |ui| {
                ui.horizontal(|ui| {
                    ui.selectable_value(&mut self.effect_panel_tab, EffectPanelTab::Effects, RichText::new("Built-in Effects").heading());
                    ui.selectable_value(&mut self.effect_panel_tab, EffectPanelTab::Custom, RichText::new("Custom Editor").heading());
                });
                ...
                .show(ui, |ui| {
                    ...
                    match self.effect_panel_tab {
                        EffectPanelTab::Effects => {
                        ...
                        EffectPanelTab::Custom => {
                            ui.vertical(|ui| {
                                // Custom effects selectors and management
                                ui.horizontal(|ui| {
                                    ui.label(RichText::new("Saved Library:").strong());

                                    let modal = self.saved_items.setup_effect_modal(ctx, &mut self.loaded_effect);
                                    if ui.button("+").on_hover_text("Add current effect to library").clicked() {
                                        modal.open();
                                    }
                                    if ui.button("-").on_hover_text("Remove selected effect").clicked() {
                                        self.saved_items.custom_effects.retain(|eff| eff != &self.loaded_effect.effect);
                                    }
                                });

                                if !self.saved_items.custom_effects.is_empty() {
                                    ui.horizontal_wrapped(|ui| {
                                        for effect in self.saved_items.custom_effects.iter() {
                                            let name = effect.name.as_deref().unwrap_or("Unnamed");
                                            if ui.selectable_value(&mut self.loaded_effect.effect, effect.clone(), name).clicked() {
                                                self.state_changed = true;
                                                self.loaded_effect.effect = effect.clone();
                                                self.loaded_effect.state = State::Queued;
                                            };
                                        }
                                    });
                                    ui.separator();
                                ...
                                crate::gui::effect_editor::show_effect_editor(ui, &mut self.loaded_effect, &mut self.state_changed);
                            });
                        }
                    }
                });
